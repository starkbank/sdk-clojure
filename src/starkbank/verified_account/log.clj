(ns starkbank.verified-account.log
  "Every time a VerifiedAccount entity is modified, a corresponding VerifiedAccount.Log
  is generated for the entity. This log is never generated by the
  user, but it can be retrieved to check additional information
  on the VerifiedAccount.

  ## Attributes:
    - `:id` [string]: unique id returned when the log is created. ex: \"5656565656565656\"
    - `:account` [VerifiedAccount]: VerifiedAccount entity to which the log refers to.
    - `:errors` [list of maps]: list of errors linked to this VerifiedAccount event, each with :code and :message keys. ex: [{:code \"keyNotFound\" :message \"The key is not registered\"}]
...
(defn query
  ...
  ## Options:
    ...
    - `:types` [list of strings, default nil]: filter retrieved maps by types. ex: \"created\" or \"processing\"
    - `:account-ids` [list of strings, default nil]: list of VerifiedAccount ids to filter retrieved maps. ex: [\"5656565656565656\", \"4545454545454545\"]
    - `:user` [Project or Organization, default nil]: Project or Organization map returned from starkbank.user/project or starkbank.user/organization. Only necessary if starkbank.settings/user has not been set.
...
(defn page
  "Receive a list of up to 100 VerifiedAccount.Log maps previously created in the Stark Bank API and the cursor to the next page.
