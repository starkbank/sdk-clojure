(ns starkbank.corporate-purchase.log
  ...
  ## Attributes (return-only):
    ...
    - `:corporate-transaction-id` [string]: transaction ID related to the CorporateCard.
    - `:errors` [list of maps]: list of maps with `:code` and `:message` pairs, linked to this CorporatePurchase event.
    - `:type` [string]: type of the CorporatePurchase event which triggered the log creation. ex: \"approved\", \"canceled\", \"confirmed\", \"denied\", \"reversed\" or \"voided\"
...
(defn query
  ...
  ## Options:
    ...
    - `:purchase-ids` [list of strings, default nil]: list of CorporatePurchase ids to filter logs. ex: [\"5656565656565656\", \"4545454545454545\"]
    - `:user` [Project or Organization, default nil]: Project or Organization map returned from starkbank.user/project or starkbank.user/organization. Only necessary if starkbank.settings/user has not been set.
...
(defn page
  "Receive a list of up to 100 Log maps previously created in the Stark Bank API and the cursor to the next page.
