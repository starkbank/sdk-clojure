(ns starkbank.split-receiver.log
  ...
  ## Attributes (return-only):
    ...
    - `:errors` [list of maps]: list of error maps linked to this SplitReceiver event.
    - `:type` [string]: type of the SplitReceiver event which triggered the log creation. ex: \"created\", \"canceled\" or \"updated\"
...
(defn query
  ...
  ## Options:
    ...
    - `:types` [list of strings, default nil]: filter for log event types. ex: \"created\", \"canceled\" or \"updated\"
    - `:receiver-ids` [list of strings, default nil]: list of SplitReceiver ids to filter logs. ex: [\"5656565656565656\", \"4545454545454545\"]
    - `:user` [Project or Organization, default nil]: Project or Organization map returned from starkbank.user/project or starkbank.user/organization. Only necessary if starkbank.settings/user has not been set.
...
(defn page
  "Receive a list of up to 100 SplitReceiver.Log maps previously created in the Stark Bank API and the cursor to the next page.
