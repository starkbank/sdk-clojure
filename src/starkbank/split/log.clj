(ns starkbank.split.log
  ...
  ## Attributes (return-only):
    ...
    - `:errors` [list of maps]: list of error maps linked to this Split event.
...
(defn query
  ...
  ## Options:
    ...
    - `:types` [list of strings, default nil]: filter for log event types. ex: \"success\" or \"failed\"
    - `:split-ids` [list of strings, default nil]: list of Split ids to filter logs. ex: [\"5656565656565656\", \"4545454545454545\"]
    - `:user` [Project or Organization, default nil]: Project or Organization map returned from starkbank.user/project or starkbank.user/organization. Only necessary if starkbank.settings/user has not been set.
...
(defn page
  "Receive a list of up to 100 Split.Log maps previously created in the Stark Bank API and the cursor to the next page.
