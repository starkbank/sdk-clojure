(ns starkbank.invoice-pull-request.log
  ...
  ## Attributes:
    ...
    - `:errors` [list of maps]: list of errors linked to this InvoicePullRequest event, each with :code and :message keys. ex: [{:code \"keyNotFound\" :message \"The key is not registered\"}]
...
(defn query
  ...
  ## Options:
    ...
    - `:request-ids` [list of strings, default nil]: list of InvoicePullRequest ids to filter retrieved maps. ex: [\"5656565656565656\", \"4545454545454545\"]
    - `:user` [Project or Organization, default nil]: Project or Organization map returned from starkbank.user/project or starkbank.user/organization. Only necessary if starkbank.settings/user has not been set.
...
(defn page
  "Receive a list of up to 100 InvoicePullRequest.Log maps previously created in the Stark Bank API and the cursor to the next page.
