...
(defn page
  ...
  ## Return:
    - dicionary with :transaction and :cursor:
  ...
  ([]
    (def page (Transaction/page))
    (def cursor (.cursor page))
    (def transactions (map java-to-clojure (.transactions page)))
    {:transactions transactions, :cursor cursor})

  ([params]
    (def java-params (clojure-query-to-java params))
    (def page (Transaction/page java-params))
    {:transactions (map java-to-clojure (.transactions page)), :cursor (.cursor page)})

  ([params, user] 
    (def java-params (clojure-query-to-java params))
    (def page (Transaction/page java-params (#'starkbank.user/get-java-user user)))
    {:transactions (map java-to-clojure (.transactions page)), :cursor (.cursor page)}))
